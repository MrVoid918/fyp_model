import numpy as np
from tracking.kalman_filter import KalmanBoxTracker_OH
from tracking.association import Association

class Sort_OH(object):

    def __init__(self, img_w, img_h, max_age=3, min_hits=3, ):
        """
    ...
    """
        ...
        to_del = []
        outside = Association.batch_outside(trks, [self.img_w, self.img_h])
        ...
        matched, unmatched_dets, unmatched_trks, occluded_trks, unmatched_gts = self.association.associate_detections_to_trackers(self, dets, trks, gts, area_avg)
        ...
        if self.frame_count <= self.min_hits:
            for i in unmatched_dets:
                ...
                if dets[i, 4] > 0.6:
                    trk = KalmanBoxTracker_OH(dets[i, :], 0, dets[i, :]) # put dets[i, :] as dummy data for last argument
            ...
            if (len(self.unmatched_before_before) != 0) and (len(self.unmatched_before) != 0) and (len(self.unmatched) != 0):
                new_trackers, del_ind = self.association.find_new_trackers_2(self.unmatched, self.unmatched_before, self.unmatched_before_before, area_avg)
                if len(new_trackers) > 0:
                    ...
                    for i, new_tracker in enumerate(new_trackers):
                        ...
                        if new_trk_certainty > 2:
                            trk = KalmanBoxTracker_OH(unm[new_tracker[0], :], 1, unmb[new_tracker[1], :])
                            self.trackers.append(trk)
