...
class JAADDataLayer(data.Dataset):
    def __init__(self, args, split):
        ...
        data_opts = {'fstride': 1,
                 ...
                 'seq_type': 'trajectory',
                 'min_track_size': self.args.enc_steps + self.args.dec_steps + 1,
    ...
    def __getitem__(self, index):
        ...
        # if index == 0:
        #     velocity = torch.zeros((4, 1))
        # else:
        #     next_bbox = torch.FloatTensor(self.data['obs_bbox'][index+1])
        #     velocity = next_bbox - obs_bbox

        # if self.args.velocity:
        #     obs_bbox = torch.cat((obs_bbox, velocity), dim=0)

        if self.args.velocity:
            vel = torch.diff(obs_bbox, dim=0)
            vel = torch.cat((torch.zeros((1, 4)), vel), dim=0)
            obs_bbox = torch.cat((obs_bbox, vel), dim=1)

            # print(obs_bbox[:, :4].shape, ped_bbox.shape)
            _pred_pos = obs_bbox[:, :4] + vel
    ...
    def get_target(self, session, start, end, observe_length, predict_length):
        ...
        for i, target_start in enumerate(range(start, end)):
            ...
            try:
                # enc_steps, dec_steps, 4 => enc_steps, dec_steps, 8
                pos = session[target_start:target_start+predict_length,:]
